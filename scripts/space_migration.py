...
Примеры использования:
    python scripts/space_migration.py --list-spaces    # Показать список доступных пространств
    python scripts/space_migration.py                  # Все подходящие пространства  
    python scripts/space_migration.py --limit 10       # Первые 10 пространств (для тестирования)
...
Примеры использования:
  ...
  %(prog)s --skip-features    # Миграция без автоматической установки возможностей
...
НОВАЯ логика миграции:
  ...
  • Автоматическая установка возможностей: Задачи, Диск, Календарь, Чат, База знаний
...
Полезная команда для начала:
  ...
  затем выберите нужные для миграции.
    ...
    parser.add_argument(
        '--skip-features',
        action='store_true',
        help='Пропустить автоматическую установку возможностей групп (Задачи, Диск, Календарь, Чат, База знаний)'
    )
    ...
    try:
        ...
        # Настраиваем автоматическую установку возможностей
        if args.skip_features:
            migrator.enable_features_update = False
            logger.info("⚠️ Автоматическая установка возможностей групп отключена")
        else:
            logger.info("🎯 Автоматическая установка возможностей групп включена")
