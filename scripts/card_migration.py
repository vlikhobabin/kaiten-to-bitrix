...
Миграция с архивными карточками (type: 3 → "Сделаны" + STATUS=5):
    python scripts/card_migration.py --space-id 426722 --include-archived
...
Просмотр с архивными (type: 3):
    python scripts/card_migration.py --space-id 426722 --list-only --include-archived

Для полной справки: python scripts/card_migration.py --help
...
7. Миграция с включением архивных карточек (type: 3):
   python scripts/card_migration.py --space-id 426722 --include-archived

8. Просмотр всех карточек включая архивные:
   python scripts/card_migration.py --space-id 426722 --list-only --include-archived
...
Если пространство не найдено в маппинге, сначала выполните: python scripts/space_migration.py --space-id <ID>
    ...
    parser.add_argument(
        '--include-archived', 
        action='store_true',
        help='Включить в миграцию карточки из финальных колонок (type: 3) - по умолчанию они пропускаются'
    )
    ...
    if args.include_archived:
        logger.info("Включение архивных карточек: ДА (type: 3 будут мигрироваться)")
    else:
        logger.info("Включение архивных карточек: НЕТ (type: 3 пропускаются)")
    ...
    if args.list_only:
        ...
        if args.include_archived:
            logger.info("   - type: 3 (финальные) -> стадия 'Сделаны' + STATUS=5 (ВКЛЮЧЕНЫ)")
        else:
